...
export function Header() {
  ...
  const iconBtn = 'text-fg-dim hover:text-fg transition-colors w-9 h-9 flex items-center justify-center'
      ...
      <header className="flex items-center justify-between px-4 sm:px-6 pt-[calc(1rem_+_env(safe-area-inset-top))] pb-4 border-b border-line">
        ...
        <div className="flex items-center gap-1 sm:gap-2 shrink-0">
          <span className="hidden sm:inline text-[11px] font-medium px-2.5 py-1 rounded-full bg-surface-hi text-fg-dim">
            {provider === 'default' ? '演示密钥（内置）' : provider.toUpperCase()}
          </span>
          {canInstall && (
            <button
          ...
          <button onClick={() => setShowHistory(true)} className={iconBtn} aria-label="历史记录">
          ...
          <ThemeToggle />
          <button onClick={() => setShowSettings(true)} className={iconBtn} aria-label="设置">
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5}
